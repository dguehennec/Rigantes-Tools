...
com.rigantestools.MainFrame.init = function() {
    /** @private */
    /** The logger. */
    this._logger = new com.rigantestools.service.Logger("MainFrame");
    ...
    /** @private */
    /** the parent */
    this._parent = window.arguments[0];
    /** @private */
    /** the player */
	this._player = this._parent.getInterface().getPlayer();
	/** @private */
...
com.rigantestools.MainFrame.selectTab = function() {
	...
	var parameters = this._parent.getMainFrameParameters();
	...
	this._parent.setMainFrameParameters(null);
};
...
com.rigantestools.MainFrame.calculateWarTimeAndShow = function() {
    ...
    try {
		...
		var attackCalculate = new com.rigantestools.service.AttackCalculate(habitats, targetLink);
		if(attackType===0) {
...
com.rigantestools.MainFrame.calculateDefenseTimeAndShow = function() {
    ...
    try {
		...
		var nbMinSpearman = this._util.getPref(com.rigantestools.constant.PREF_NB_MIN_SPEARMAN_DEFENSE);
		if (nbMinSpearman === '') {
			nbMinSpearman = 300;
		}
		var nbMinCrossbowman = this._util.getPref(com.rigantestools.constant.PREF_NB_MIN_CROSSBOWMAN_DEFENSE);
		if (nbMinCrossbowman === '') {
			nbMinCrossbowman = 300;
		}
		var nbMinScorpionRider = this._util.getPref(com.rigantestools.constant.PREF_NB_MIN_SCORPIONRIDER_DEFENSE);
		...
		if (habitats.length > 0) {
			for (index = 0; index < habitats.length; index++) {
				...
				if (habitat.link !== targetLink) {
					var spearmanCount = Math.max(0, habitat.getUnitCount(com.rigantestools.constant.UNITTYPE.SPEARMAN) - Number(nbMinSpearman));
					var crossbowmanCount = Math.max(0, habitat.getUnitCount(com.rigantestools.constant.UNITTYPE.CROSSBOWMAN) - Number(nbMinCrossbowman));
					var scorpionRiderCount = Math.max(0, habitat.getUnitCount(com.rigantestools.constant.UNITTYPE.SCORPIONRIDER) - Number(nbMinScorpionRider));
					var totalUD = spearmanCount + crossbowmanCount + scorpionRiderCount;
					// check duration with spearman
					var duration = habitat.getUnitDurationTo(com.rigantestools.constant.UNITTYPE.SPEARMAN, targetLink);
					var currentTime = new Date().getTime();
					var ArrivalDate = currentTime + duration * 1000;
					if ((spearmanCount === 0) || (duration !== 0) && (ArrivalDate > warTime)) {
						spearmanCount = 0;
						duration = habitat.getUnitDurationTo(com.rigantestools.constant.UNITTYPE.CROSSBOWMAN, targetLink);
						// check duration with crossbowman
						ArrivalDate = currentTime + duration * 1000;
						if ((crossbowmanCount === 0) || (duration !== 0) && (ArrivalDate > warTime)) {
							crossbowmanCount = 0;
							duration = habitat.getUnitDurationTo(com.rigantestools.constant.UNITTYPE.SCORPIONRIDER, targetLink);
...
{
	var coeffsUnit = com.rigantestools.constant.BATTLEVALUES[unit];
...
com.rigantestools.MainFrame.initializeWarInProgressInformation = function() {
    try {
    	...
    	for ( var indexHab = 0; indexHab < habitats.length; indexHab++) {
    		...
    		var habitatTransits = habitat.getHabitatTransits(com.rigantestools.constant.TRANSITTYPE.ATTACKER, true);
    	...
    	for ( var indexHab = 0; indexHab < habitats.length; indexHab++) {
    		var habitatTransits = habitats[indexHab].getHabitatTransits(com.rigantestools.constant.TRANSITTYPE.ATTACKER,false);
    	...
    	for(var key in habitatTransitsList) {
    	    if (habitatTransitsList.hasOwnProperty(key)) {
        		...
        		for(var indexHabTrans=0; indexHabTrans<habitatTransits.length; indexHabTrans++){
        			...
        			// add PA
        			var pa = habitatTransits[indexHabTrans].getResourceCount(com.rigantestools.constant.RESOURCETYPE.ARGENT);
        			nbPA += pa;
        			// add unitsCountSwordman
        			var unitsCountSwordman = habitatTransits[indexHabTrans].getUnitCount(com.rigantestools.constant.UNITTYPE.SWORDMAN);
        			nbSwordman += unitsCountSwordman;
        			// add unitsCountArcher
        			var unitsCountArcher = habitatTransits[indexHabTrans].getUnitCount(com.rigantestools.constant.UNITTYPE.ARCHER);
        			nbArcher += unitsCountArcher;
        			// add unitsCountSwordman
        			var unitsCountLancer = habitatTransits[indexHabTrans].getUnitCount(com.rigantestools.constant.UNITTYPE.LANCER);
...
com.rigantestools.MainFrame.initializePlayerInformation = function() {
	try {
		...
		// create tabs
		var area = this._util.getPref(com.rigantestools.constant.PREF_CASTLES_AREA);
		var provArea = [];
		if(area!=="" && area !== null) {
			provArea = area.split('#');
		}
		for ( var indexProv = 0; indexProv < provArea.length+2; indexProv++) {
			...
			for(var indexCol=0; indexCol<cols.length; indexCol = indexCol+2){
			    treecols.push([ "xul:treecol" , { 'label' : cols[indexCol], 'flex' : cols[indexCol+1], 'name' : (indexCol/2), 'ignoreincolumnpicker' : true, 'persist' : 'width ordinal', 'class' : 'sortDirectionIndicator', 'sortDirection' : 'natural', 'indexProv' : indexProv, 'click' : function (evt) { 
                    com.rigantestools.MainFrame.sortcolumnPlayerTree(evt.target.getAttribute("indexProv"), evt.target); 
                }}]);
			}
			 // generate tabpanel element
		...
		for ( var indexHab = 0; indexHab < habitats.length; indexHab++) {
			var habitat = habitats[indexHab];
			var paCount = habitat.getResourceCount(com.rigantestools.constant.RESOURCETYPE.ARGENT);
			var swordmanCount = habitat.getUnitCount(com.rigantestools.constant.UNITTYPE.SWORDMAN);
			var archerCount = habitat.getUnitCount(com.rigantestools.constant.UNITTYPE.ARCHER);
			var lancerCount = habitat.getUnitCount(com.rigantestools.constant.UNITTYPE.LANCER);
			var spearmanCount = habitat.getUnitCount(com.rigantestools.constant.UNITTYPE.SPEARMAN);
			var crossbowmanCount = habitat.getUnitCount(com.rigantestools.constant.UNITTYPE.CROSSBOWMAN);
			var scorpionRiderCount = habitat.getUnitCount(com.rigantestools.constant.UNITTYPE.SCORPIONRIDER);
...
com.rigantestools.MainFrame.refreshPlayerTree = function(indexProv) {
	...
	var crossbowmanCount = 0;
	var scorpionRiderCount = 0;
	var items = com.rigantestools.MainFrame.currentHabitats[indexProv];
	...
	if(this._util.getAttribute('rigantestools-player-nbCasltes'+indexProv, 'value')!==items.length) {
	    this._util.setAttribute('rigantestools-player-nbCasltes'+indexProv, 'value', items.length);
	            this._util.setAttribute('rigantestools-player-nbCaption'+indexProv, 'value', this.getNbCaption(paCount, com.rigantestools.MainFrame.currentHabitats[0].length) + " (" + paCount + " "+this._util.getBundleString("mainframe.player.PA")+")");
		var nbUO = swordmanCount + archerCount + lancerCount;
...
com.rigantestools.MainFrame.onSimulateWarButtonClick = function(evt) {
	com.rigantestools.MainFrame.clearSimulationTab();
	this._util.setAttribute('rigantestools-simulationAttaqueSpearman', 'value', '');
	this._util.setAttribute('rigantestools-simulationAttaqueSwordman', 'value', com.rigantestools.MainFrame.simulationSwordmanCount);
	this._util.setAttribute('rigantestools-simulationAttaqueArcher', 'value', com.rigantestools.MainFrame.simulationArcherCount);
	...
	this._util.setAttribute('rigantestools-simulationAttaqueLancer', 'value', com.rigantestools.MainFrame.simulationLancerCount);
...
com.rigantestools.MainFrame.onGenerateWarInProgressButtonClick = function(evt) {
	...
	if(selectedAttacks<com.rigantestools.MainFrame._generatedAttacks.length) {
		this._util.copieToClipboard(com.rigantestools.MainFrame._generatedAttacks[selectedAttacks]);
	}
};
...
com.rigantestools.MainFrame.onGenerateCasltesLinkPlayerButtonClick = function(evt) {
	var generatedCastlesLink = com.rigantestools.MainFrame._player.name + "\n" + com.rigantestools.MainFrame._player.link;
	var area = this._util.getPref(com.rigantestools.constant.PREF_CASTLES_AREA);
	var provArea = [];
	if(area!=="" && area !== null){
		provArea = area.split('#');
	}
	var itemsProv = com.rigantestools.MainFrame.currentHabitats;
...
com.rigantestools.MainFrame.onGenerateCastlesUOPlayerButtonClick = function(evt) {
	...
	for ( var indexProv = 1; indexProv < itemsProv.length; indexProv++) {
		...
		if (nbCastles > 0) {
			...
			generatedCastlesUO += this._util.getBundleString("mainframe.player.totalCaption") + " : " + com.rigantestools.MainFrame.getNbCaption(paCount, itemsProv[0].length) + "\n\n";
		}
	}
...
com.rigantestools.MainFrame.onSelectTreeSimulationClick = function(evt) {
	...
	if (selectedTreeItemIndex === -1 || selectedTreeItemIndex >= com.rigantestools.MainFrame.currentUnitsSimulation.length) {
		return;
	}
	if(selectedTreeItemIndex===(com.rigantestools.MainFrame.currentUnitsSimulation.length-1)) {
...
com.rigantestools.MainFrame.activateWarTreeContext = function(evt) {
	...
	var start = {};
    var end = {};
	var datetime = com.rigantestools.MainFrame.currentDateTime.getTime();
	...
	for (var rangIndex = 0; rangIndex < numRanges; rangIndex++){
        ...
        for (var index = start.value; index <= end.value; index++){
            ...
            if(index >=0 && index < com.rigantestools.MainFrame.currentHabitatsWar.length) {
                var item = com.rigantestools.MainFrame.currentHabitatsWar[index];
...
com.rigantestools.MainFrame.selectTreeContext = function(evt, name) {
	...
	com.rigantestools.MainFrame.refreshWarTable();
	com.rigantestools.MainFrame.sortcolumnWarTree(com.rigantestools.MainFrame._currentHabitatsWarColumnTreeSort, true);
	com.rigantestools.MainFrame.selectItems(selectedItems);
};
...
com.rigantestools.MainFrame.sortcolumnWarTree = function(column, withoutSortDirection) {
	var lastColumn = com.rigantestools.MainFrame._currentHabitatsWarColumnTreeSort;
	...
	if (column !== null) {
		...
		com.rigantestools.MainFrame._currentHabitatsWarColumnTreeSort = column;
	}
	...
	case "castle":
		if (sort === "ascending") {
			com.rigantestools.MainFrame.currentHabitatsWar.sort(function(a, b) {
				if (b.name > a.name) {
					return -1;
				}
	...
	case "slowUnit":
		if (sort === "ascending") {
			com.rigantestools.MainFrame.currentHabitatsWar.sort(function(a, b) {
				var unitb = com.rigantestools.MainFrame.getUnitWeight(b.selectedUnit);
				var unita = com.rigantestools.MainFrame.getUnitWeight(a.selectedUnit);
				return unita - unitb;
			});
		} else {
			com.rigantestools.MainFrame.currentHabitatsWar.sort(function(a, b) {
	...
	com.rigantestools.MainFrame.refreshWarTree();
};
...
com.rigantestools.MainFrame.sortcolumnDefenseTree = function(column) {
	var lastColumn = com.rigantestools.MainFrame._currentHabitatsDefenseColumnTreeSort;
	if (lastColumn !== null && lastColumn !== column) {
		lastColumn.removeAttribute("sortDirection");
	}
	var name = "duration";
	...
	if (column !== null) {
		name = column.getAttribute("name");
		sort = column.getAttribute("sortDirection");
		com.rigantestools.MainFrame._currentHabitatsDefenseColumnTreeSort = column;
	}
	if (sort !== 'ascending') {
		sort = "ascending";
	}
	...
	case "castle":
		if (sort === "ascending") {
			com.rigantestools.MainFrame.currentHabitatsDefense.sort(function(a, b) {
				if (b.name > a.name) {
					return -1;
				}
				else if (b.name < a.name) {
	...
	com.rigantestools.MainFrame.refreshDefenseTree();
};
/**
...
com.rigantestools.MainFrame.sortcolumnAttackDefenseSlowTree = function(column) {
	var lastColumn = com.rigantestools.MainFrame._currentHabitatsAttackDefenseSlowColumnTreeSort;
	if (lastColumn !== null && lastColumn !== column) {
		lastColumn.removeAttribute("sortDirection");
	}
	var name = "duration";
	...
	if (column !== null) {
		name = column.getAttribute("name");
		sort = column.getAttribute("sortDirection");
		com.rigantestools.MainFrame._currentHabitatsAttackDefenseSlowColumnTreeSort = column;
	}
	if (sort !== 'ascending') {
		sort = "ascending";
	}
	...
	case "castle":
		if (sort === "ascending") {
			com.rigantestools.MainFrame.currentHabitatsAttackDefenseSlow.sort(function(a, b) {
	...
	}com.rigantestools.MainFrame.refreshAttackDefenseSlowTree();
};
/**
